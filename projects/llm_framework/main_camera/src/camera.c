...
static void* CameraCaptureThread(void* pParam)
{
    ...
    while (Camera->State == CAMERA_SATTE_CAP) {
        ...
        Camera->pCallback(Camera->pCameraBuffer[DeQueueBuf.index].pStart, Camera->Width, Camera->Height,
                          Camera->pCameraBuffer[DeQueueBuf.index].Length, Camera->ctx_);
...
static void CameraSetCtx(struct Camera_t* Camera, void* ctx)
{
    Camera->ctx_ = ctx;
}
...
int CameraOpenFrom(Camera_t* Camera)
{
    ...
    Camera->CameraSetCtx             = CameraSetCtx;
    Camera->is_alloc                 = 0;
    return 0;
...
Camera_t* CameraOpen(const char* pDevName, int width, int height, int fps)
{
    int Ret          = -1;
    ...
    /* Set capture format */
    TvFmt.type                = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    TvFmt.fmt.pix.width       = width;
    TvFmt.fmt.pix.height      = height;
    TvFmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
    TvFmt.fmt.pix.field       = V4L2_FIELD_INTERLACED;
